...
@device_model_router.delete("/{device_model_id}")
...
@device_model_router.get("/get-device-models-with-beverages/{dev_model_id}",
                         response_model=DeviceModelSchemaWithBeverages)
...
@device_model_router.get("/all-beverages-for-every-device-model/",
                         response_model=list[DeviceModelSchemaWithBeverages])
def get_all_beverages_for_every_device_model():
    return DeviceModelsController.get_all_beverages_for_every_device_model()


@device_model_router.delete("/delete-beverage-for-device-model/{beverage_id}/{device_model_id}")
def delete_beverage_for_device_model(beverage_id: int, device_model_id: int):
    try:
        DeviceModelsController.delete_beverage_for_device_model(beverage_id=beverage_id,
                                                                device_model_id=device_model_id)
        return {"message": f"Beverage for device model successfully deleted."}
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
