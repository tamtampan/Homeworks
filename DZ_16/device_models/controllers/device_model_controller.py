...
class DeviceModelsController:
    ...
    def add_new_beverage_to_device_model(dev_model_id: int, beverage_id: int):
        ...
        try:
            ...
            flag = DeviceModelsService.add_new_beverage_to_device_model(dev_model_id=dev_model_id,
    ...
    @staticmethod
    def get_all_beverages_for_every_device_model():
        try:
            device_model_ids = DeviceModelsService.get_all_device_model_ids_that_produces_beverages()
            result = [DeviceModelsService.get_device_model_by_id(device_model_id) for device_model_id in
                      device_model_ids]
            if len(result) == 0:
                return "No device models that produces beverages."
            for device_model in result:
                beverage_ids = DeviceModelsService.get_all_beverages_ids_for_device_model(device_model.device_model_id)
                beverages = []
                for beverage in beverage_ids:
                    beverages.append(BeverageServices.get_beverage_by_id(beverage.beverage_id))
    ...
    @staticmethod
    def delete_beverage_for_device_model(beverage_id: int, device_model_id: int):
        # Da li je uopste trebalo da proveravam da li postoji device model i beverage u bazi? Jer trebalo bi da mozemo
        # da obrisemo iako ih vise nema u bazi ali postoje u ovoj tabeli...
        # s druge strane ovako hvatamo greske preciznije...?
        try:
        ...
        try:
            ...
            raise HTTPException(status_code=400, detail='Beverage with this id does not exist')
        try:
            DeviceModelsService.delete_beverage_for_device_model(beverage_id, device_model_id=device_model_id)
        except Exception as e:
            raise e
